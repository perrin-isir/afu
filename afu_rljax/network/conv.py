...
from afu_rljax.network.initializer import DeltaOrthogonal
...
class SACEncoder(hk.Module):
    ...
    def __call__(self, x):
        ...
        for _ in range(self.num_layers - 1):
            x = hk.Conv2D(
                self.num_filters, kernel_shape=3, stride=1, 
                padding="VALID", w_init=w_init)(x)
            x = nn.leaky_relu(x, self.negative_slope)
        # Flatten the feature map.
        return hk.Flatten()(x)
...
class SACDecoder(hk.Module):
    ...
    def __call__(self, x):
        ...
        x = hk.Conv2DTranspose(
            self.state_space.shape[2], kernel_shape=4, stride=2, 
            padding="VALID", w_init=w_init)(x)
        return x
